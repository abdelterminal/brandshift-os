...
export default async function NewDocumentPage({ searchParams }: { searchParams: Promise<{ company?: string }> }) {
  ...
  const [companies, letterhead, projects] = await Promise.all([
    listCompanies(session.actor), getLetterhead(session.actor), listProjects(session.actor)
  ]);
  return <DocumentComposer kind="invoice" companies={companies.map(c => ({ id: c.id, label: c.name }))}
