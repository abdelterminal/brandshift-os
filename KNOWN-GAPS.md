...
| `Combobox`'s `clearLabel`/`openLabel` don't reach a screen reader when the combobox sits inside a `Field` | Found building the task drawer's assignee picker. Both render as a real `aria-label` attribute on the Clear and trigger buttons, but Base UI separately wires an `aria-labelledby` on each pointing at the `Field`'s own label -- and per the ARIA spec, `aria-labelledby` wins, so both buttons are announced by the field's label ("Assignee") rather than what they actually do. The buttons still work; only the announced name is wrong. Confirmed against a real accessibility-tree snapshot, not assumed. Worth fixing in `combobox.tsx` itself once a design for it exists, since every future `Combobox` inside a `Field` inherits the same gap -- not attempted here to avoid changing a shared primitive for the sake of one new caller. |
