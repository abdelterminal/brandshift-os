...
| Thing | Why |
|---|---|
| A department cannot be renamed, described after the fact, archived, or given a lead | Create was the actual gap -- a fresh organization has nowhere to put anyone otherwise -- and it is fixed. Everything past creation is a different, smaller screen: the schema already carries `description` and `leadUserId` for it, and `archivedAt` already exists (`listDepartments` already filters it out), but nothing writes to any of the three yet. |
...
| The task drawer's own Start / Complete / Report blocker / Clear blocker have no project-membership check | Found while adding the board's drag-and-drop, which *is* restricted to a project's own lead or contributor (`saveBoardChanges` checks `project_members` before writing anything). The drawer's four single-task actions predate that check and still do not have it -- `requireUserForAction()` only proves someone is signed into the organization, not that they are on this particular project. Anyone who can see a task today can already act on it from the drawer regardless of membership; the board is more careful than the screen it sits beside, not the other way around. Worth closing by giving the drawer the same check, not by loosening the board's. |
