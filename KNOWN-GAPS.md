...
| The Notion workspace's SOP text is in; template, KPI and review text is not | An earlier export was the hub page only. A later full API export carried the **SOP Library** -- twelve real procedures, imported into `sops` / `sop_steps` by `db:import:sops` (see `MIGRATION.md`). That export holds no objective, KPI, template or weekly-review rows -- those databases are empty in it -- so those parts of the model are still real and their content still absent, and none has been invented. Projects, Tasks and Team in the export are name-only and were left to `db:migrate:mongo`, which carries them with real dates and owners. |
| The imported SOPs have no owner and no department | `db:import:sops` writes both as null. The Notion "Owner" is a role title ("Marketing Manager", "Production Lead & Senior Video Editor", "CEO"), not one of this org's departments; the Notion SOP-owner relation points at Team pages that are not people in the `mediast` org. Set them by hand in the SOP editor, or teach the importer a mapping, once the real team exists here. |
...
| The task drawer's own Start / Complete / Report blocker / Clear blocker / reassign have no project-membership check | Found while adding the board's drag-and-drop, which *is* restricted to a project's own lead or contributor (`saveBoardChanges` checks `project_members` before writing anything). The drawer's single-task actions predate that check and still do not have it -- `requireUserForAction()` only proves someone is signed into the organization, not that they are on this particular project. `assignTask`, added later, deliberately matched the existing four rather than becoming a fifth inconsistency of its own. Anyone who can see a task today can already act on it from the drawer regardless of membership; the board is more careful than the screen it sits beside, not the other way around. Worth closing by giving the drawer the same check, not by loosening the board's. |
