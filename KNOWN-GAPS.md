...
| A stage change does not push to the project channel live | It records a `project.stageChanged` activity event, which the channel feed interleaves -- so it shows up next time the channel is opened, not the instant it happens. `publishChannelChange` needs the channel id, and the action has the project id; wiring the lookup is small and was left for when somebody notices the lag. |
| Nothing advances a project's stage automatically | Moving between stages is always a manual drag or select. Phase 2 (the playbook) is where "finishing the last task of a stage proposes the next one" belongs. |
