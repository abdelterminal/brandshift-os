...
| Deliverables still don't show on the pipeline board, or on a client's own page | `/today` lists a person's own open deliverables across every project (`listMyDeliverables`); the CRM company page now lists a client's projects, task counts and team (`listProjects(actor, {companyId})`). Still missing: the pipeline board, and deliverables specifically on the company page -- an account lead sees a client's projects and who's on them, but not what's due to be delivered. Each is a read against a table that already has the right index. |
| Existing projects have no client, and nothing assigns them in bulk | `projects.company_id` is a new column; every project that existed before it shipped starts `null`. A project on the delivery flow (`stage` set) with no client shows a "No client" flag on `/work` and on its own page; a project with no `stage` at all shows nothing, on the assumption it is internal work -- which is a guess for a project migrated in with a client but no stage. Fixing one is a manual pick from the project's own header; there is no "assign these ten at once" tool. |
...
| The composer sits at the end of the column, not pinned to the viewport | The shell now owns the scroll region -- see DECISIONS.md -- so the structural blocker this row used to name is gone. What is left is the actual reason the pinned version was reverted the first time: it covered the newest message on desktop and landed underneath the bottom nav on a phone. That is a composer-specific layout problem (padding, z-index, safe-area) to solve on its own, with the responsive sweep watching, not something the shell change did for free. |
