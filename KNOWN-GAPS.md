...
| Deliverables still don't show on the pipeline board, or across one client | `/today` now lists a person's own open deliverables across every project (`listMyDeliverables`). Still missing: the pipeline board, and a cross-project "everything with this client" view for someone who isn't the assignee -- an account lead checking a client's whole workload, say. Each is a read against a table that already has the right index. |
